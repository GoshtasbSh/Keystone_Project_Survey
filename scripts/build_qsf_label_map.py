"""Generate api/qsf_labels.py — the authoritative answer labels, from the QSF.
...
def main() -> None:
    ...
    data = build(qsf_path)
    # Emitted as a Python module, not JSON: a data file is only bundled into a
    # Vercel function if vercel.json lists it under includeFiles, and a silently
    # missing map would make the label fix a no-op in production. An imported
    # module is always bundled.
    header = (
        '"""Generated by scripts/build_qsf_label_map.py — do not edit by hand.\n\n'
        'Authoritative answer labels taken from the Qualtrics survey definition\n'
        '(.qsf). Maps both the export\'s VariableNaming text and its numeric\n'
        'recode codes back to the choice text the respondent actually saw.\n'
        'Regenerate after any change to the questionnaire.\n"""\n\n'
        'QSF_LABELS = '
    )
    OUT.write_text(header + repr(data) + '\n')
